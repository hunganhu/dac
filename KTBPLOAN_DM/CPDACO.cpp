                          ...
                          char *bank_code, char *ole_db_str, int gender, char *error)
{
  ...
  AnsiString msn = static_cast<AnsiString>(msno);
  ...
  if(bank!= "054")
    throw cc_error(1, msn, input_time);
...
//Copy JCIC data
      ...
      copy_table(command, "BAM086", BAM086, msn, jcic_date, input_time);
      copy_table(command, "STM007", STM001, msn, jcic_date, input_time);
      copy_table(command, "JAS002", JAS002, msn, jcic_date, input_time);
...
//Calculate npv
      ...
      throw cc_error(0, msn, input_time);
    };
  }
  catch(cc_error &Err){
    ...
    if(Err.error_code() > 100)
      store_result(command, Err.idn(), Err.app_date(),
                   Err.error_code(), Err.ShowMessage(), 0, 
                   unsecured_balance_opt, 0, 0, unsecured_balance_dac,
                   0, 0, pb, special_note, version, false);
  }
  catch(Exception &E){
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
  ...
  sql_stmt +="AND msn = :msn AND Input_Time = :input_time ";
...
//    if(E.Message.SubString(0,16) == "μLak ‥o°￡ ﹐eRAai");
  ...
  sql_stmt += "(MSN CHAR(14), IDN CHAR(11), ";
  ...
  sql_stmt = "INSERT INTO KRM023_TMP (MSN, IDN, ";
  sql_stmt += "MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT, PAYMENT_AMT, ";
  sql_stmt += "CASH, PAY_CODE, SPREAD_PAYMENT) SELECT MSN, IDN, ";
...
//  sql_stmt += "WHEN LEN(BILL_DATE) = 7 THEN CONVERT(INT,LEFT(BILL_DATE, 3)) * 12 + CONVERT(INT, SUBSTRING(BILL_DATE, 4, 2)) - 1 ";
  ...
  sql_stmt = "INSERT INTO KRM023_TMP1 (MSN, IDN, MON_SINCE, ISSUE, ISSUE_NAME, ";
  sql_stmt += "LIMIT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT) ";
  sql_stmt += "SELECT MSN, IDN, MON_SINCE, ISSUE, ISSUE_NAME, SUM(CONVERT(INT, LIMIT)), ";
  ...
  sql_stmt += "GROUP BY MSN, IDN, MON_SINCE, ISSUE, ISSUE_NAME";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt += "KRM023_TMP1.ISSUE = A.ISSUE AND ";
  sql_stmt += "KRM023_TMP1.MSN = A.MSN";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt = "INSERT INTO " + krm023 + "(MSN, IDN, ";
  sql_stmt += "MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, ";
  sql_stmt += "CASH, PAY_CODE, SPREAD_PAYMENT) SELECT MSN, IDN, MON_SINCE, ";
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
  ...
  sql_stmt = "CREATE TABLE KRM023_TMP ";
  sql_stmt += "(MSN CHAR(14), IDN CHAR(11), INPUT_TIME CHAR(12), INQUIRY_DATE CHAR(8), ";
  sql_stmt += "MON_SINCE INT, ISSUE CHAR(3), ISSUE_NAME CHAR(40), LIMIT CHAR(5),";
  ...
  sql_stmt = "INSERT INTO KRM023_TMP (MSN, IDN, INPUT_TIME, INQUIRY_DATE, ";
  sql_stmt += "MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT, ";
  sql_stmt += "CASH, PAY_CODE) SELECT MSN, IDN, INPUT_TIME, INQUIRY_DATE, ";
  sql_stmt += "(CASE WHEN LEFT(BILL_DATE,1) = '*' THEN NULL ";
  sql_stmt += "WHEN LEN(BILL_DATE) = 4 AND LEFT(BILL_DATE,1) BETWEEN '1' AND '9' ";
  sql_stmt += "THEN CONVERT(INT, LEFT(BILL_DATE, 2)) * 12 + CONVERT(INT, RIGHT(BILL_DATE, 2)) ";
...
//      if(E.Message.SubString(0,16) == "無法 卸除 資料表");
  ...
  sql_stmt += " (MSN Char(14), TYPE CHAR(1), E_DATE Char(7), ";
  ...
  sql_stmt += " SELECT MSN, 'D', DELINQUENT_DATE, NULL FROM " + ori_table;
  ...
  sql_stmt = "INSERT INTO " + dest_table;
  sql_stmt += " SELECT MSN, 'B', BAD_CHECK_DATE, NULL FROM " + ori_table;
  ...
  sql_stmt = "INSERT INTO " + dest_table;
  sql_stmt += " SELECT MSN, 'R', REJECT_DATE, NULL FROM " + ori_table;
  ...
  sql_stmt = "INSERT INTO " + dest_table;
  sql_stmt += " SELECT MSN, 'S', STOP_CARD_DATE, NULL FROM " + ori_table;
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
	...
	try{
  	sql_stmt = "CREATE TABLE IDN_LIST_TMP (MSN CHAR(14), HIT INT);";//KRM001 = 2, KRM023 = 1, BAM086 = 4, STM001 = 8, JAS002 = 16
    sql_stmt = sql_stmt.UpperCase();
  	command->CommandText = sql_stmt;
  	command->Execute();
	}
...
//    if(E.Message.SubString(0, 20) == "資料庫已經有一個名為");
	...
	try{
  	sql_stmt = "CREATE TABLE IDN_LIST (MSN CHAR(14), HIT INT);";
    sql_stmt = sql_stmt.UpperCase();
  	command->CommandText = sql_stmt;
  	command->Execute();
	}
...
//    if(E.Message.SubString(0, 20) == "資料庫已經有一個名為");
  ...
  sql_stmt = "INSERT INTO IDN_LIST_TMP SELECT DISTINCT MSN, 1 FROM " + KRM023 + " ";
  command->CommandText = sql_stmt;
  sql_stmt = sql_stmt.UpperCase();
  command->Execute();

  sql_stmt = "INSERT INTO IDN_LIST_TMP SELECT DISTINCT MSN, 2 FROM " + KRM001 + " ";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "INSERT INTO IDN_LIST_TMP SELECT DISTINCT MSN, 4 FROM " + BAM086 + " ";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "INSERT INTO IDN_LIST_TMP SELECT DISTINCT MSN, 8 FROM " + STM001 + " ";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
  ...
  sql_stmt = "CREATE TABLE TMP(MSN CHAR(14), MON INT, V1 FLOAT, ";
  sql_stmt += "V2 FLOAT, V3 FLOAT);";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE TMP1(MSN CHAR(14), MON INT, V1 FLOAT);";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  
  sql_stmt = "CREATE TABLE TMP2(MSN CHAR(14), ISSUER CHAR(3));";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE TMP3(MSN CHAR(14), MON INT, V1 FLOAT);";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE OPEN_CARD (MSN CHAR(14), ISSUE CHAR(3), MON_SINCE INT);";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE OPEN_LINE (MSN CHAR(14), ISSUE CHAR(3), MON_SINCE INT, ";
  sql_stmt += "Cards INT, BUCKET INT)";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE LATEST_STMT_MON (MSN CHAR(14), ISSUE CHAR(3), ";
  sql_stmt += "MON_SINCE INT)";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE LATEST_LINE (MSN CHAR(14), ISSUE CHAR(3), ";
  sql_stmt += "MON_SINCE INT, MOB INT)";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "CREATE TABLE OPEN_ISSUER (MSN CHAR(14), ISSUE CHAR(3), MON INT);";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
};
...
void prepare_2xx_infra(TADOCommand *command, int now)
{
  ...
  sql_stmt = "INSERT INTO OPEN_LINE SELECT MSN, ISSUE, MON_SINCE, Count(*), NULL FROM ";
  sql_stmt += "OPEN_CARD GROUP BY MSN, ISSUE, MON_SINCE;";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "DECLARE @i INT SET @i = 0 WHILE @i<=13 BEGIN INSERT INTO ";
  sql_stmt += "OPEN_LINE SELECT MSN, (CASE ";
  ...
  sql_stmt += "OPEN_LINE ON A.MSN = OPEN_LINE.MSN AND ";
  ...
  sql_stmt = "INSERT INTO LATEST_STMT_MON SELECT MSN, ISSUE, MAX(MON_SINCE) ";
  sql_stmt += "FROM OPEN_LINE WHERE MON_SINCE <= :now GROUP BY MSN, ISSUE;";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Parameters->ParamValues["now"] = now;
  command->Execute();

  sql_stmt = "INSERT INTO LATEST_LINE SELECT A.MSN, A.ISSUE, A.MON_SINCE, A.BUCKET ";
  sql_stmt += "FROM OPEN_LINE AS A INNER JOIN LATEST_STMT_MON AS B ";
  sql_stmt += "ON A.MSN = B.MSN AND A.ISSUE = B.ISSUE AND A.MON_SINCE = B.MON_SINCE;";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
};
...
{
	AnsiString sql_stmt;
	sql_stmt = "SELECT HIT FROM IDN_LIST WHERE MSN = :msn;";
	sql_stmt = sql_stmt.UpperCase();
	query->Close();
	query->SQL->Clear();
...
//    if(E.Message.SubString(0, 20) == "資料庫已經有一個名為");
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_01, 1 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_02, 2 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_03, 3 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_04, 4 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_05, 5 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_06, 6 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_07, 7 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_08, 8 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_09, 9 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_10, 10 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_11, 11 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "insert into y_BAM086_tmp(MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle) ";
  sql_stmt += "select ";
  sql_stmt += "MSN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, pc_12, 12 - :now + cast(isnull(data_yyy,0) as int) * 12 + cast(isnull(data_mm,0) as int) + 1 ";
  sql_stmt += "from " + bam086;
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["now"] = now;
                ...
                unsigned int &max_line, unsigned int &unsecured_balance_dac)
{
  ...
  double spread_payment_rev_balance = 0;
  ...
  if ((flag & 0x1)==0)
    ...
    sql_stmt += " WHERE :now - MON_SINCE = 1 AND MSN = :msn";
    sql_stmt = sql_stmt.UpperCase();
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
    ...
    sql_stmt += "(MSN CHAR(14), ISSUE CHAR(3), MON INT)";
    sql_stmt = sql_stmt.UpperCase();
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
    ...
    sql_stmt = "INSERT INTO KRM023_RANGE_TMP(MSN, ISSUE, MON) ";
    sql_stmt += "SELECT MSN, ISSUE, MAX(MON_SINCE) FROM " + krm023 + " ";
    sql_stmt += "WHERE :now - MON_SINCE <= 3 ";
    sql_stmt += "GROUP BY MSN, ISSUE";
    sql_stmt = sql_stmt.UpperCase();
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
    ...
    sql_stmt = "SELECT SUM(CASE WHEN PAY_CODE IN ('C', 'D', 'E', 'F') THEN ";
    sql_stmt += "PAYMENT_AMT ELSE 0 END) AS REV_BAL, SUM(SPREAD_PAYMENT) ";
    sql_stmt += "AS SPREAD_PAYMENT, SUM(CASE WHEN ((PAY_CODE IN ";
    sql_stmt += "('C', 'D', 'E', 'F')) OR (CASH = 'Y')) ";
    sql_stmt += "THEN SPREAD_PAYMENT ELSE 0 END) AS REV_SPREAD, ";
    sql_stmt += "SUM(CONVERT(FLOAT, LIMIT)) AS LINE ";
    sql_stmt += "FROM " + krm023 + " AS A INNER JOIN KRM023_RANGE_TMP AS B ";
    sql_stmt += "ON A.MSN = B.MSN AND A.ISSUE = B.ISSUE AND A.MON_SINCE = B.MON ";
    sql_stmt += "WHERE A.MSN = :msn";
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
    query->Parameters->ParamValues["msn"] = msn;
    query->Open();
    if(!query->FieldValues["REV_BAL"].IsNull()){
      credit_card_rev_balance_opt = query->FieldValues["REV_BAL"];
      credit_card_rev_balance_opt *= 1000;
    };
    if(!query->FieldValues["REV_SPREAD"].IsNull()){
      spread_payment_rev_balance = query->FieldValues["REV_SPREAD"];
      spread_payment_rev_balance *= 1000;
    };
    if(!query->FieldValues["SPREAD_PAYMENT"].IsNull()){
  ...
  if ((flag & 0x2)==0)
    ...
    try{
      sql_stmt = "CREATE TABLE IND_TMP (MSN CHAR(14), Mon int);";
      sql_stmt = sql_stmt.UpperCase();
      query->Close();
      query->SQL->Clear();
      query->SQL->Add(sql_stmt);
...
//    if(E.Message.SubString(0, 20) == "資料庫已經有一個名為");
    ...
    sql_stmt = "INSERT INTO IND_TMP SELECT msn, MIN(Start_Mon_Since) FROM ";
    sql_stmt += krm001 + " WHERE msn = :msn GROUP BY msn;";
    sql_stmt = sql_stmt.UpperCase();
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
  ...
  if(flag & 0x4){
/*----FS044----*/
    sql_stmt ="SELECT msn, SUM(CASE WHEN PASS_DUE_AMT > 0 THEN 1 ELSE 0 END) AS CNT ";
    ...
    if(count > 0)
      exclusion |= 0x10;
    sql_stmt = "SELECT msn, SUM(CASE WHEN ACCOUNT_CODE = 'Y' AND ";
    ...
    if(count > 0)
      exclusion |= 0x80;
    sql_stmt = "SELECT msn, MAX(Bucket) as cnt FROM y_BAM086_tmp ";
    sql_stmt += "WHERE msn = :msn GROUP BY msn;";
    sql_stmt = sql_stmt.UpperCase();
    query->Close();
    query->SQL->Clear();
    ...
    sql_stmt += "WHERE msn = :msn AND ACCOUNT_CODE = 'Y';";
    sql_stmt = sql_stmt.UpperCase();
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
    ...
    sql_stmt += "FROM " + bam085 + " ";
    sql_stmt += "WHERE MSN = :msn AND ACCOUNT_CODE NOT IN ('Y', 'K') AND ";
  ...
  unsecured_balance_dac = cash_card_balance + credit_card_rev_balance_opt + spread_payment_rev_balance;
...
//	  query->Parameters->ParamValues["now"] = now;
  ...
  if(unsecured_balance_opt >= 1000000)
    exclusion |= 0x1000;
  return exclusion;
};
                  ...
                  int gender, int now, unsigned int exclusion, unsigned int data_flag)
{
  ...
  bool with_gender = true;
	...
	try{
	  sql_stmt = "CREATE TABLE PDACO_V1_00_CAL (MSN CHAR(14), ";
...
//    if(E.Message.SubString(0, 20) == "資料庫已經有一個名為");
  ...
  sql_stmt = "INSERT INTO PDACO_V1_00_CAL(MSN, HIT, EXCLUSION, SEX) ";
	sql_stmt += "VALUES (:msn, :hit, :exclusion, :gender);";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["msn"] = msn;
...
//Init variables
      ...
      sql_stmt += "WHERE PDACO_V1_00_CAL.msn = :msn AND ";
      sql_stmt += "PDACO_V1_00_CAL.msn = A.msn AND ";
      sql_stmt += "A.msn = B.msn AND ";
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
	  ...
	  sql_stmt += "(msn CHAR(14), Mon INT, V1 FLOAT, V2 FLOAT, V3 FLOAT);";
    sql_stmt = sql_stmt.UpperCase();
	  command->CommandText = sql_stmt;
	  command->Execute();

	  sql_stmt = "CREATE TABLE TMP1 ";
	  sql_stmt += "(msn CHAR(14), Mon INT, V1 FLOAT);";
    sql_stmt = sql_stmt.UpperCase();
	  command->CommandText = sql_stmt;
	  command->Execute();
...
/*---Start making FS014---*/
    ...
    sql_stmt +="SELECT msn, 9, COUNT(DISTINCT ISSUE), NULL, NULL ";
		sql_stmt +=("FROM " + krm023 + " ");
    sql_stmt +="WHERE (:now - Mon_Since)<= 9 AND ";
//    sql_stmt +="(:now1 - Mon_Since) > 0 AND ";
    sql_stmt +="Pay_Code IN ('A', 'B') GROUP BY msn;";
    sql_stmt = sql_stmt.UpperCase();
	  command->CommandText = sql_stmt;
	  command->Parameters->ParamValues["now"] = now;
//	  command->Parameters->ParamValues["now1"] = now;
    ...
    sql_stmt +="WHERE A.msn = PDACO_V1_00_CAL.msn AND Mon=9;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Execute();
...
/*---Start making FS016---*/
		sql_stmt ="INSERT INTO TMP ";
		sql_stmt +="SELECT msn, 3, COUNT(DISTINCT ISSUE), NULL, NULL ";
		sql_stmt +=("FROM " + krm023 + " ");
		sql_stmt +="WHERE (:now - Mon_Since)<= 3 AND ";
//		sql_stmt +="(:now1 - Mon_Since) > 0 AND ";
		sql_stmt +="CASH = 'Y' GROUP BY msn ";
    sql_stmt = sql_stmt.UpperCase();
	  command->CommandText = sql_stmt;
	  command->Parameters->ParamValues["now"] = now;
//	  command->Parameters->ParamValues["now1"] = now;
		...
		sql_stmt +="WHERE A.msn = PDACO_V1_00_CAL.msn AND Mon=3;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Execute();
...
/*---Start making FS101---*/
		...
		sql_stmt +="SELECT msn, @i, COUNT(DISTINCT ISSUE) ";
		sql_stmt +=("FROM " + krm023 + " ");
		sql_stmt +="WHERE (:now - Mon_Since)<= @i ";
//		sql_stmt +="AND (:now1 - Mon_Since) > 0 ";
		sql_stmt +="GROUP BY msn SET @i = @i + 6 END;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Parameters->ParamValues["now"] = now;
//	  command->Parameters->ParamValues["now1"] = now;
...
//		sql_stmt +="AND (:now1 - Mon_Since) > 0 ";
		sql_stmt +="GROUP BY msn SET @i = @i + 3 END;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Parameters->ParamValues["now"] = now;
//	  command->Parameters->ParamValues["now1"] = now;
    ...
    sql_stmt = sql_stmt.UpperCase();
	  ...
	  command->Execute();
		...
		sql_stmt +="WHERE A.msn = PDACO_V1_00_CAL.msn AND Mon=6;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Execute();
...
/*---Start making FS212_1K---*/
		...
		sql_stmt +="BEGIN ";
		sql_stmt +="INSERT INTO TMP ";
		sql_stmt +="SELECT msn, @I, COUNT(*), NULL, NULL ";
		sql_stmt +="FROM " + krm023 + "  AS A ";
		sql_stmt +="WHERE Issue IN ";
		sql_stmt +="(SELECT Issue FROM Latest_Line ";
		sql_stmt +="WHERE  (MON_SINCE - MOB + 1) >=  (:now - 6) AND msn = A.msn) AND ";
    ...
    sql_stmt = sql_stmt.UpperCase();
	  ...
	  command->Execute();
		...
		sql_stmt +="WHERE A.msn = PDACO_V1_00_CAL.msn AND A.Mon = 3;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Execute();
		...
		sql_stmt +="FROM TMP1 AS A ";
		sql_stmt +="WHERE A.msn = PDACO_V1_00_CAL.msn AND A.Mon = 6;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Execute();
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
		...
		sql_stmt = "CREATE TABLE FS_ISSUE(msn Char(14), MON Int, ISSUE Char(3));";
    sql_stmt = sql_stmt.UpperCase();
	  command->CommandText = sql_stmt;
	  command->Execute();
...
/*---Start making FS102---*/
		...
		sql_stmt +="SELECT msn, Mon_Since, Issue  ";
		sql_stmt +="FROM " + krm023 + "  ";
    ...
    sql_stmt = sql_stmt.UpperCase();
		...
		sql_stmt +="BEGIN ";
		sql_stmt +="INSERT INTO TMP ";
		sql_stmt +="SELECT DISTINCT msn, @i, 0, NULL, NULL ";
		sql_stmt +=("FROM " + krm023 + "  ");
		sql_stmt +="WHERE (:now - Mon_Since)<= @i ";
//		sql_stmt +="AND (:now1 - Mon_Since) > 0 ";
		sql_stmt +="GROUP BY msn SET @i = @i + 3 END;";
...
//	  command->Parameters->ParamValues["now1"] = now;
		...
		sql_stmt +="FROM TMP1 AS A ";
		sql_stmt +="WHERE A.msn = TMP.msn AND TMP.MON = A.MON;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
 		command->Execute();
...
/*---Start making FS031---*/
		  sql_stmt ="INSERT INTO TMP1 ";
		  sql_stmt +="SELECT msn, NULL, COUNT(*) ";
      ...
      sql_stmt = sql_stmt.UpperCase();
  	  ...
  	  command->Parameters->ParamValues["now"] = now;
		  ...
		  sql_stmt +="FROM TMP1 AS A ";
  		sql_stmt +="WHERE A.msn = PDACO_V1_00_CAL.msn ";
      sql_stmt = sql_stmt.UpperCase();
	  	command->CommandText = sql_stmt;
 		  command->Execute();
    };
/*----App_Last_Month_Bucket----*/
    ...
    sql_stmt = "INSERT INTO TMP(MSN, MON) SELECT msn, MAX(MON_SINCE) ";
    sql_stmt += "FROM " + krm023 + " ";
    sql_stmt += "GROUP BY MSN";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
    ...
    sql_stmt = "INSERT INTO TMP1 SELECT A.msn, NULL, MAX(BUCKET_EF_1K) ";
    sql_stmt += "FROM " + krm023 + " AS A INNER JOIN TMP AS B ";
    sql_stmt += "ON A.MSN = B.MSN AND A.MON_SINCE = B.MON ";
    sql_stmt += "GROUP BY A.msn;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
 		command->Execute();

		sql_stmt = "UPDATE PDACO_V1_00_CAL SET APP_LAST_MONTH_BUCKET = A.V1 ";
		sql_stmt += "FROM TMP1 AS A WHERE PDACO_V1_00_CAL.msn = A.msn;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
 		command->Execute();
...
/*----FS205_1K----*/
    ...
    sql_stmt = sql_stmt.UpperCase();
	  ...
	  command->Execute();
		...
		sql_stmt +="INSERT INTO TMP1 ";
		sql_stmt +="SELECT msn, 3, COUNT(*) ";
		sql_stmt +="FROM " + krm023 + "  AS A ";
		sql_stmt +="WHERE Issue IN ";
		sql_stmt +="(SELECT Issue FROM Latest_Line ";
		sql_stmt +="WHERE MOB <= 12 AND msn = A.msn) AND ";
		sql_stmt +="Pay_Code in ('C', 'D', 'E','F') AND ";
		sql_stmt +="Payment_Amt > 1 AND ";
...
/*---Start making MS117---*/
    sql_stmt ="INSERT INTO TMP1 ";
    sql_stmt+="SELECT msn, 6, MIN(Payment_amt/CONVERT(decimal(13,5),Limit)) ";
    sql_stmt+=("FROM " + krm023 + " ");
    sql_stmt+="WHERE (:now - Mon_Since)<= 6 AND ";
    sql_stmt+="      (:now1 - Mon_Since) > 0 AND ";
    sql_stmt+="       Pay_Code IN ('C', 'D') AND ";
    sql_stmt+="       Limit <> 0 GROUP BY msn;";
    sql_stmt = sql_stmt.UpperCase();
		command->CommandText = sql_stmt;
	  command->Parameters->ParamValues["now"] = now;
	  command->Parameters->ParamValues["now1"] = now;
...
/*---Start making FS005---*/
    sql_stmt ="INSERT INTO TMP1 ";
    sql_stmt +="SELECT msn, Mon_Since, 1 ";
    sql_stmt +=("FROM " + krm023 + " ");
    sql_stmt +="WHERE Bucket_f_1K >=2 ";
    sql_stmt +="GROUP BY msn, Mon_Since  ";
    sql_stmt = sql_stmt.UpperCase();
	  command->CommandText = sql_stmt;
	  command->Execute();

    sql_stmt ="INSERT INTO TMP ";
    sql_stmt +="SELECT msn, 3, SUM(V1), NULL, NULL ";
    sql_stmt +="FROM TMP1 ";
...
//Model calculation
    ...
    command->Parameters->ParamValues["msn"] = msn;
	  ...
	  sql_stmt = "SELECT SCORE FROM PDACO_V1_00_CAL WHERE msn = :msn;";
	  sql_stmt = sql_stmt.UpperCase();
	  query->Close();
	  query->SQL->Clear();
	  query->SQL->Add(sql_stmt);
...
void prescreen_out(unsigned int filter_flag, unsigned int insufficiency_flag, AnsiString msn,
                   AnsiString input_time)
{
  if (filter_flag & 0x20)
    throw cc_error(101, msn, input_time);
  if (filter_flag & 0x4)
    throw cc_error(102, msn, input_time);
  if (filter_flag & 0x40)
    throw cc_error(103, msn, input_time);
  if (filter_flag & 0x80)
    throw cc_error(104, msn, input_time);
  if (filter_flag & 0x10)
    throw cc_error(105, msn, input_time);
  if (filter_flag & 0x100)
    throw cc_error(107, msn, input_time);
  if (filter_flag & 0x400)
    throw cc_error(108, msn, input_time);
  if (filter_flag & 0x800)
    throw cc_error(110, msn, input_time);
  if (filter_flag & 0x1000)
    throw cc_error(116, msn, input_time);
  if (insufficiency_flag & 0x01)
    throw cc_error(111, msn, input_time);
  if (insufficiency_flag & 0x02)
    throw cc_error(112, msn, input_time);
  if (insufficiency_flag & 0x04)
    throw cc_error(113, msn, input_time);
  if (insufficiency_flag & 0x10)
    throw cc_error(114, msn, input_time);
  if (insufficiency_flag & 0x20)
    throw cc_error(115, msn, input_time);
  if (insufficiency_flag & 0x08)
    throw cc_error(201, msn, input_time);
};
...
{
	AnsiString sql_stmt;
	sql_stmt = "SELECT * FROM APPLICANT WHERE MSN = :msn AND INPUT_TIME = :input_time";
  sql_stmt = sql_stmt.UpperCase();
  query->Close();
  query->SQL->Clear();
  query->SQL->Add(sql_stmt);
  ...
  if(query->RecordCount ==0)
    ...
    if((query->FieldValues["APPLICATION_AMOUNT"].IsNull()) || (query->FieldValues["APPLICATION_AMOUNT"]<150000))
    	throw cc_error(202, msn, input_time);
    else
                    ...
                    bool &wi004_12m_isnull)
{
  pdaco_score = pdaco_1_00(query, command,
                           ...
                           msn, gender, now, exclusion, data_flag);
  ci_variables(query, command, now, krm023, bam085, msn, data_flag,
                  ...
                  bool &wi008_9m_isnull, bool &wi002_12m_isnull, bool &wi004_12m_isnull)
{
  ...
  try{
    sql_stmt = "CREATE TABLE CI (msn CHAR(14), MS047 float, MS080 float, ";
  ...
  //    if(E.Message.SubString(0, 20) == "資料庫已經有一個名為");
	...
	sql_stmt = "INSERT INTO CI(msn) VALUES (:msn)";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Parameters->ParamValues["msn"] = msn;
  command->Execute();
  ...
  sql_stmt = "INSERT INTO TMP(msn, V1) ";
  sql_stmt += "SELECT msn, SUM(CONVERT(INT, CONTRACT_AMT)) FROM " + bam085;
  sql_stmt += " WHERE purpose_code = '1' GROUP BY msn;";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//MS080
  ...
  sql_stmt = "INSERT INTO TMP(msn, V1) ";
  sql_stmt += "SELECT msn, SUM(CAST(CONTRACT_AMT AS FLOAT) - CAST(LOAN_AMT AS FLOAT) - CAST(PASS_DUE_AMT AS FLOAT)) ";
  sql_stmt += "FROM " + bam085 + " WHERE ACCOUNT_CODE != 'Y' GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET MS080 = V1 FROM TMP AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//MS093
  ...
  sql_stmt += "END) FROM " + bam085 + " GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET MS093 = V1 FROM TMP AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//MS094
  ...
  sql_stmt = "INSERT INTO TMP(msn, V1) ";
  sql_stmt += "SELECT msn, SUM(PAYMENT_AMT) FROM " + krm023;
  ...
  sql_stmt = "INSERT INTO TMP1(msn, V1) ";
  sql_stmt += "SELECT msn, AVG(V1) FROM TMP ";
  sql_stmt += "GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
/*Anne's code
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET MS094 = V1 FROM TMP1 AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//MS105
  ...
  sql_stmt = "INSERT INTO TMP(msn, V1) ";
  sql_stmt += "SELECT msn, sum( convert(float, loan_amt))+sum(convert(float, pass_due_amt)) ";
  sql_stmt += "FROM " + bam085 + " where account_code='Y' GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET MS105 = V1 FROM TMP AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//WI002_12M
  ...
  sql_stmt = "INSERT INTO TMP1(msn, MON, V1) ";
  sql_stmt += "SELECT msn, MON_SINCE, COUNT(*) FROM " + krm023;
  sql_stmt += " WHERE CONVERT(INT, LIMIT) > 100 GROUP BY msn, MON_SINCE";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  
  sql_stmt = "INSERT INTO TMP(msn, V1) ";
  sql_stmt += "SELECT msn, AVG(CONVERT(FLOAT,V1)) FROM TMP1 WHERE :now - MON BETWEEN 1 AND 12 ";
  sql_stmt += "GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET WI002_12M = V1 FROM TMP AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//WI004_12M
  ...
  sql_stmt = "INSERT INTO TMP1(msn, MON, V1) ";
  sql_stmt += "SELECT msn, MON_SINCE, SUM(CONVERT(INT, LIMIT)) ";
  sql_stmt += "FROM " + krm023 + " GROUP BY msn, MON_SINCE";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "INSERT INTO TMP(msn, V1) ";
  sql_stmt += "SELECT msn, AVG(V1) FROM TMP1 WHERE :now - MON BETWEEN 1 AND 12 ";
  sql_stmt += "GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET WI004_12M = V1 FROM TMP AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//WI006_9M
  ...
  sql_stmt = "INSERT INTO TMP3(msn) ";
  sql_stmt += "SELECT msn FROM " + krm023 + " GROUP BY msn ";
  sql_stmt += "HAVING MAX(BUCKET_EF_1K) = 0";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  
  sql_stmt = "INSERT INTO TMP(msn, MON, V1, V2) ";
  sql_stmt += "SELECT msn, MON_SINCE, 0, 0 FROM " + krm023 + " WHERE msn IN ";
  sql_stmt += "(SELECT msn FROM TMP3) GROUP BY msn, MON_SINCE";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "INSERT INTO TMP1(msn, MON, V1) ";
  sql_stmt += "SELECT msn, MON_SINCE, SUM(PAYMENT_AMT) FROM " + krm023;
  sql_stmt += " WHERE PAY_CODE IN ('C', 'D') AND ";
  sql_stmt += "msn IN (SELECT msn FROM TMP3) GROUP BY msn, MON_SINCE";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();

  sql_stmt = "UPDATE TMP SET V1 = A.V1 FROM TMP1 AS A ";
  sql_stmt += "WHERE A.msn = TMP.msn AND A.MON = TMP.MON";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt +="INNER JOIN TMP ON A.msn = TMP.msn AND A.MON = TMP.MON - 1";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt = "INSERT INTO TMP3 (msn, V1) ";
  sql_stmt += "SELECT msn, AVG(V2) FROM TMP WHERE :now - MON BETWEEN 1 AND 9 ";
  sql_stmt += "GROUP BY msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET WI006_9M = A.V1 FROM TMP3 AS A WHERE A.msn = CI.msn";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
...
//WI008_3M
  ...
  sql_stmt = "INSERT INTO TMP(msn, MON, V2) ";
  sql_stmt += "SELECT msn, :NOW - MON_SINCE, SUM(CASE WHEN PAY_CODE IN ('C', 'D', 'E', 'F') THEN PAYMENT_AMT ELSE 0 END) ";
  sql_stmt += "FROM " + krm023 + " GROUP BY msn, MON_SINCE";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Parameters->ParamValues["now"] = now;
  ...
  sql_stmt = "INSERT INTO TMP1(msn, MON, V1)";
  sql_stmt += "SELECT msn, MON_SINCE, SUM(PAYMENT_AMT) FROM " + krm023;
  sql_stmt += " WHERE PAY_CODE IN ('A', 'B') GROUP BY msn, MON_SINCE";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt += "FROM TMP1 AS A INNER JOIN TMP AS B ON A.msn = B.msn ";
  ...
  sql_stmt = "INSERT INTO TMP1(msn, MON, V1) ";
  sql_stmt += "SELECT msn, 3, SUM(V1) FROM TMP WHERE MON BETWEEN 1 AND 3 ";
  sql_stmt += "GROUP BY msn";    
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET WI008_3M = A.V1 FROM TMP1 AS A WHERE A.msn = CI.msn ";
...
//WI008_9M  
  sql_stmt = "INSERT INTO TMP1(msn, MON, V1) ";
  sql_stmt += "SELECT msn, 9, SUM(V1) FROM TMP WHERE MON BETWEEN 1 AND 9 ";
  sql_stmt += "GROUP BY msn";    
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();
  ...
  sql_stmt = "UPDATE CI ";
  sql_stmt += "SET WI008_9M = A.V1 FROM TMP1 AS A WHERE A.msn = CI.msn ";
  sql_stmt += "AND A.MON = 9";  
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
  command->Execute();  
  
  sql_stmt = "SELECT * FROM CI WHERE msn = :msn";
  sql_stmt = sql_stmt.UpperCase();
  query->Close();
  query->SQL->Clear();
  query->SQL->Add(sql_stmt);
...
//Model calculated
  if(normal){
    ...
    command->Parameters->ParamValues["approved_amount"] = approved_amount_dac_rounded;
    ...
    command->Parameters->ParamValues["module_code"] = 0;
    command->Execute();
  }
  else{
    int result_output;
    ...
    command->Parameters->ParamValues["note"] = note;
    command->Parameters->ParamValues["version"] = version;
    command->Parameters->ParamValues["module_code"] = result_code;
    switch (result_code){
      case 0:
      case 1:
        result_output = -1;
...
double get_pdaco_score(TADOQuery *query, const AnsiString &msn)
{
  AnsiString sql_stmt;
  double return_value;
  sql_stmt = "SELECT PDACO_SCORE FROM input_data_step1 WHERE IDN = :msn";
  sql_stmt = sql_stmt.UpperCase();
  query->Close();
  query->SQL->Clear();
  query->SQL->Add(sql_stmt);
                    ...
                    const AnsiString &input_time, double ci_score)
{
	AnsiString sql_stmt;
  try{
	  sql_stmt = "CREATE TABLE CI_SCORE (MSN CHAR(14), INPUT_TIME CHAR(12), ";
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
	...
	sql_stmt = "INSERT INTO CI_SCORE(MSN, INPUT_TIME, SCORE) VALUES(";
	sql_stmt += ":msn, :input_time, :score)";
  sql_stmt = sql_stmt.UpperCase();
  command->CommandText = sql_stmt;
	command->Parameters->ParamValues["msn"] = msn;
  command->Parameters->ParamValues["input_time"] = input_time;
...
{
  AnsiString sql_stmt = "SELECT PB FROM PDACO_V1_00_CAL WHERE msn = :msn;";
  double pb;
  sql_stmt = sql_stmt.UpperCase();
  query->Close();
  query->SQL->Clear();
                        ...
                        const AnsiString &msn, const AnsiString &input_time)
{
  ...
  sql_stmt = "SELECT MONTHLY_INCOME FROM APPLICANT WHERE MSN = :msn AND ";
...
void vam102_message(TADOQuery *query, const AnsiString &msn,
                    const AnsiString &jcic_date, const AnsiString &input_time,
                    AnsiString &note)
{
  AnsiString sql_stmt;
  sql_stmt = "SELECT NOTE FROM VAM102 WHERE MSN = :msn AND ";
  ...
  while(!query->Eof){
    if(!query->FieldValues["NOTE"].IsNull()){
      note += static_cast<AnsiString>(query->FieldValues["NOTE"]).Trim();
      note += " ";
    };
    query->Next();
  };
                                ...
                                const AnsiString &msn, const AnsiString &input_time)
{
  ...
  if((filter_flag & 0x1) || (filter_flag & 0x2) || (filter_flag & 0x200) || (filter_flag & 0x8)){
    ...
    return_value |= 0x8;
    if(data_flag & 0x1){
      sql_stmt += "SELECT MSN, COUNT(DISTINCT ISSUE) AS V1 ";
      sql_stmt += "FROM " + krm023 + " ";
      sql_stmt += "WHERE Bucket_ef_1K >=1 AND ";
    ...
    if(data_flag & 0x8){
		  sql_stmt ="SELECT msn, COUNT(*) AS V1 ";
  		sql_stmt +="FROM " + stm001 + " ";
	  	sql_stmt +="WHERE Item_List IS NOT NULL AND  ";
		  sql_stmt +="Item_List <> '' AND  ";
  		sql_stmt +="(:now - QUERY_MON_SINCE) <= 12 ";
...
//    store_data_insufficiency(query, msn, input_time, data_flag,
//                             delinquent_lines, inquiries, stop_cards);
...
{
  unsigned int with_krm023;
  unsigned int with_krm001;
  if(data_flag & 0x01){
    with_krm023 = 0x01;
    with_krm001 = 0x0;
  }
  ...
  AnsiString sql_stmt;
  sql_stmt = "INSERT INTO DATA_INSUFFICIENCY(",
  sql_stmt += "MSN, INPUT_TIME, WITH_KRM023, WITH_KRM021_OR_BAM086, ";
  sql_stmt += "CREDIT_CARD_DELINQUENT, INQUIRIES, STOP_CARD) VALUES";
  sql_stmt += "(:msn, :input_time, :with_krm023, :with_krm001, ";
  sql_stmt += ":credit_card_delinquent, :inquiries, :stop_card)";
  sql_stmt = sql_stmt.UpperCase();
  query->Close();
  query->SQL->Clear();
  query->SQL->Add(sql_stmt);
  query->Parameters->ParamValues["msn"] = msn;
  query->Parameters->ParamValues["input_time"] = input_time;
  query->Parameters->ParamValues["with_krm023"] = with_krm023;
  query->Parameters->ParamValues["with_krm001"] = with_krm001;
  query->Parameters->ParamValues["credit_card_delinquent"] = delinquent_cards;
  query->Parameters->ParamValues["inquiries"] = inquiries;
  query->Parameters->ParamValues["stop_card"] = stop_cards;
  query->ExecSQL();
};
