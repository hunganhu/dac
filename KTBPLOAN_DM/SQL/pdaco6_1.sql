...
/* CREATE_PROCEDURE_PREPARE_JCIC_DATA */
 CREATE PROCEDURE PREPARE_PDACO_JCIC_DATA
  (@MSN VARCHAR(20),
   @INPUT_TIME CHAR(12))
  AS
 INSERT INTO #PDACO_V6_1(MSN, INPUT_TIME)
  VALUES (@MSN, @INPUT_TIME)
 ...
 INSERT INTO #KRM037_DEDUP(MSN, INQUIRY_DATE, IDN, IDN_BAN, BILL_DATE, ISSUE, ISSUE_NAME, CARD_TYPE, PERM_LIMIT, TEMP_LIMIT, CASH_LIMIT, PAYABLE, CASH_LENT, LAST_PAYA, REVOL_BAL, PAY_STAT, PAY_CODE, REVOL_RATE, PRE_OWED, DEBT_CODE, CLOSE_CODE, CLEAR_DATE, INPUT_TIME)
  SELECT MSN, INQUIRY_DATE, IDN, IDN_BAN, BILL_DATE, ISSUE, ISSUE_NAME, CARD_TYPE, PERM_LIMIT, TEMP_LIMIT, CASH_LIMIT, PAYABLE, CASH_LENT, LAST_PAYA, REVOL_BAL, PAY_STAT, PAY_CODE, REVOL_RATE, PRE_OWED, DEBT_CODE, CLOSE_CODE, CLEAR_DATE, INPUT_TIME
  FROM KRM037
  WHERE ISSUE != 'TOT'
    AND MSN = @MSN AND INPUT_TIME = @INPUT_TIME
  GROUP BY MSN, INQUIRY_DATE, IDN, IDN_BAN, BILL_DATE, ISSUE, ISSUE_NAME, CARD_TYPE, PERM_LIMIT, TEMP_LIMIT, CASH_LIMIT, PAYABLE, CASH_LENT, LAST_PAYA, REVOL_BAL, PAY_STAT, PAY_CODE, REVOL_RATE, PRE_OWED, DEBT_CODE, CLOSE_CODE, CLEAR_DATE, INPUT_TIME
 UPDATE #KRM021_DEDUP
 ...
 INSERT INTO #KRM023_TMP (MSN, INQUIRY_DATE, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT)
 ...
 INSERT INTO #KRM023_TMP1 (MSN, INQUIRY_DATE, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT)
   SELECT MSN, INQUIRY_DATE, IDN, MON_SINCE, ISSUE, ISSUE_NAME, SUM(CONVERT(INT, LIMIT)), SUM(PAYMENT_AMT),
           MAX(CASH), MAX(CASE WHEN PAY_CODE = 'X' THEN '0' ELSE PAY_CODE END), SUM(SPREAD_PAYMENT)
   FROM #KRM023_TMP
   GROUP BY MSN, INQUIRY_DATE, IDN, MON_SINCE, ISSUE, ISSUE_NAME
 UPDATE #KRM023_TMP1
   SET CASH = A.CASH
 ...
 UPDATE #KRM023_DEDUP
  SET PAYMENT_AMT =
   ...
   ELSE 0 END) * POWER(10, ISNULL(LEFT(PAYMENT,2),0)-1) / 1000.0;
 INSERT INTO #KRM023_DEDUP (MSN, INQUIRY_DATE, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT)
   SELECT MSN, INQUIRY_DATE, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT
   FROM #KRM023_TMP1
 ...
 BEGIN
   ...
   WHERE (INQUIRY_MON - A.MON_SINCE) = @I
 SET @I = @I - 1
 END;
 ...
 INSERT INTO #BASE_TMP(MSN, AVAIL_FLAG)
  VALUES (@MSN, 0);
 INSERT INTO #BASE_TMP(MSN, AVAIL_FLAG)
   SELECT DISTINCT MSN, 1
   FROM #KRM023_DEDUP;
 INSERT INTO #BASE_TMP(MSN, AVAIL_FLAG)
   SELECT DISTINCT MSN, 2
   FROM #KRM021_DEDUP;
 INSERT INTO #BASE_TMP(MSN, AVAIL_FLAG)
   SELECT DISTINCT MSN, 4
   FROM #BAM086_DEDUP;
 INSERT INTO #BASE_TMP(MSN, AVAIL_FLAG)
   SELECT DISTINCT MSN, 8
   FROM #JAS002_T
 INSERT INTO #BASE_TMP(MSN, AVAIL_FLAG)
   SELECT DISTINCT MSN, 16
   FROM #STM007_DEDUP;
 INSERT INTO #BASE (MSN, AVAIL_FLAG)
   SELECT MSN, SUM(AVAIL_FLAG)
   FROM #BASE_TMP
  GROUP BY MSN;
 UPDATE #PDACO_V6_1
   SET AVAIL_FLAG = A.AVAIL_FLAG
   FROM #BASE A
   WHERE A.MSN = #PDACO_V6_1.MSN;
 UPDATE #PDACO_V6_1
   SET KRM023_HIT = 1
 ...
 DELETE FROM #TMP;
 INSERT INTO #TMP (IDN, V1, V2)
   SELECT IDN, MIN(INQUIRY_MON) - MIN(MON_SINCE), MAX(INQUIRY_MON) - MAX(MON_SINCE)
   FROM #KRM023_DEDUP
   GROUP BY IDN;
 UPDATE #PDACO_V6_1
   ...
   WHERE A.IDN = #PDACO_V6_1.IDN;
 INSERT INTO #TMP1 (IDN, V1)
   SELECT IDN, (CASE WHEN (INQUIRY_MON - MIN(START_MON_SINCE)) < 7 THEN 1 ELSE 0 END)
   FROM #KRM021_DEDUP
 ...
 INSERT INTO #TMP(IDN, MON, V1)
    ...
    WHERE PAY_CODE IN ('C', 'D', 'E', 'F')
      AND MON_SINCE >= INQUIRY_MON - 3
      AND MON_SINCE <= INQUIRY_MON
 ...
 UPDATE #PDACO_V6_1
       ...
       WHERE EXISTS (SELECT *
                     ...
                     WHERE ACCOUNT_CODE = 'Y'
                       AND #PDACO_V6_1.IDN = S.IDN);
 UPDATE #PDACO_V6_1 SET MS101 = MS093 + (MS094 + MS105) * 0.35;
...
/*BEGINNING BAM086-RELATED *****************************************/
 ...
 /* CDEF_FLAG_1M   */
 UPDATE #PDACO_V6_1
    SET	CDEF_FLAG_1M = (SELECT (CASE WHEN PAY_CODE IN ('C','D','E','F') THEN 1 ELSE 0 END)
                        FROM	#KRM023_DEDUP AS A
