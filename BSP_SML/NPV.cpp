...
void loc::get_product_feature()
{
  ...
  double teaser_rate = product[_test_cell-1][0];
  double go_to_rate = product[_test_cell-1][1];
  _mgmt_fee = product[_test_cell-1][4];
...
void loc::attrition(double pb, bool voluntary)
{
  ...
  for(int i = 1; i <=_periods; ++i){
    ...
    else if(_test_cell<9){
      if(i==13)
        _series[i][4] -= 0.3 * _series[i-1][4];
    }
    else if(_test_cell<13){
       if(i==7)
        _series[i][4] -= 0.3 * _series[i-1][4];
    }
    else{
...
void il::get_periods()
{
  _periods = product[_test_cell-1][3];
}

void il::get_product_feature()
{
  int periods;
  double apr = product[_test_cell-1][1];
  _mgmt_fee = product[_test_cell-1][4];
  _mgmt_fee_periods = product[_test_cell-1][5];
...
il::il(int test_cell, double gav, double nav, double existing_mortgage,
       AnsiString zip, double principal, double balance,
...
//    double taken_amount = effective_amount(loan_amount);
    double pb;
    if(secured){
      _series[0][7] = (taken_amount * (_commission + Set_lien_fee + (1 - _leverage)) + 2.25 ) * (-1); //對保2.25 + 代書 3 = 5.25; 代書 3 與set_lien_fee因向申貸人收費而打平
