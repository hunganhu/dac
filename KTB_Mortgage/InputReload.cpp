...
#pragma resource "*.dfm"
TfrmReloadInput *frmReloadInput;
extern AnsiString case_sn;
//---------------------------------------------------------------------------
__fastcall TfrmReloadInput::TfrmReloadInput(TComponent* Owner)
  : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TfrmReloadInput::btnExitClick(TObject *Sender)
{
...
void __fastcall TfrmReloadInput::FormShow(TObject *Sender)
{
  btnClear->Click();
}
//---------------------------------------------------------------------------

void __fastcall TfrmReloadInput::btnClearClick(TObject *Sender)
{
  ...
  medtGAV->Clear();
  medtNAV->Clear();
  medtMonthlyMortgagePayment->Clear();
  medtOwnerID->Clear();
...
void __fastcall TfrmReloadInput::ckbCoAppClick(TObject *Sender)
{
  if(ckbCoApp->Checked)
...
void __fastcall TfrmReloadInput::ckbGuarantorClick(TObject *Sender)
{
...
void __fastcall TfrmReloadInput::cmbInterestSectionsChange(TObject *Sender)
{
...
unsigned int __fastcall TfrmReloadInput::check_keyin(AnsiString &msg)
{
  ...
  if(!check_amount(medtMonthlyMortgagePayment->Text))
    throw keyin_error(31);
    ...
    const AnsiString &guarantor_inquiry_date)
{
  ...
  sql_stmt += "BRANCH, EMP_ID, AUDITOR, MONTHLY_PAYMENT) VALUES( ";
  ...
  sql_stmt += ":grace_period, :app_fee, :owner_id, :owner_name, :land_num, ";
  sql_stmt += ":relationship, :gav, :nav, :col_qualified, :premium_col, ";
  sql_stmt += ":branch, :emp_id, :auditor, :monthly_payment)";
  ...
  command->Parameters->ParamValues["product_type"] = 2;
  ...
  command->Parameters->ParamValues["monthly_payment"] = medtMonthlyMortgagePayment->Text.Trim();
  command->Execute();
};
//---------------------------------------------------------------------------

void __fastcall TfrmReloadInput::btnScoreClick(TObject *Sender)
{
  AnsiString message;
...
//         get_store_jcic_data(Data->ejcic_connection, Data->ejcic_query, Data->command, query_sn_guarantor, medtGuarantorID->Text, case_sn);
      ...
      frmReloadInput->Hide();
      Application->CreateForm(__classid(TfrmSNReminder2), &frmSNReminder2);
      frmSNReminder2->lblCase_SN->Caption = case_sn;
      frmSNReminder2->ShowModal();
    };
  }
