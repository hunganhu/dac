...
//Get loan amount, tenure, apr, and fee
        get_product_parameters(query, idn, input_time,
                               apr, app_amount, terms, application_fee);
...
//          trial_amount = fsc_lendable;
        ...
        ploan  application(lendable_amount, apr, terms, application_fee,
                           pdaco_score, product_type);
//        double pb;

        double min_rate;
        double npv = application.npv(pb, min_rate);

        store_result(command, idn, input_time, 0, "", app_amount, unsecured_balance_opt,
    ...
    if(Err.error_code() > 100)
      store_result(command, Err.idn(), Err.app_date(),
                   Err.error_code(), Err.ShowMessage(), 0, unsecured_balance_opt,
...
//    if(E.Message.SubString(0,16) == "無法 卸除 資料表");
    ...
    sql_stmt += "ON A.IDN = B.IDN AND A.ISSUE = B.ISSUE AND A.MON_SINCE = B.MON ";
    sql_stmt += "WHERE A.IDN = :idn";
    query->Close();
    query->SQL->Clear();
    query->SQL->Add(sql_stmt);
    query->Parameters->ParamValues["idn"] = idn;
                   ...
                   AnsiString input_time)
{
  ...
  if (filter_flag & 0x1000)
    throw cc_error(116, idn, input_time);
  ...
  if (insufficiency_flag & 0x10)
    throw cc_error(114, idn, input_time);
  if (insufficiency_flag & 0x20)
    throw cc_error(115, idn, input_time);
...
/*    if(npv > -1000 && npv < 0){
    ...
    sql_stmt += "SPECIAL_NOTE, UNSECURED_BALANCE_DAC, VERSION, MODULE_CODE) ";
    sql_stmt += "VALUES(:idno, :input_time, :approved_amount, :pb, :npv, ";
    sql_stmt += ":message, :code, :min_rate, :unsecured_balance, ";
    sql_stmt += ":approved_amount_fsc, :note, :unsecured_balance_dac, :version, :module_code);";
    sql_stmt = sql_stmt.UpperCase();
    int output_npv;
    if(npv < 0)
      output_npv = 0;
