...
" update #krm034_dedup "
"    set mon_since = (case when len(bill_date) = 4 and left(bill_date,1) between '1' and '9'"
...
"                        and mon_since = cast(left(s.bill_date, 3) as int) * 12 * cast(substring(s.bill_date, 4, 2) as int)"
...
" update #base "
"     set ind001 = "
"         (select (case when @now - min(start_mon_since) >= 7 then 1 else 0 end) "
"          from #krm021_dedup s "
"          where #base.idn = s.idn "
"            and s.end_mon_since > @now "
"            and s.m_s = 'Y' "
"          group by idn) "
"     where exists (select * "
"                   from #krm021_dedup s "
...
"  update #base "
"     set ind001 = ind001 + "
"         (select (case when (@now - min(mon_since) >= 6) and "
"                            (@now - max(mon_since) < 6) then 1 else 0 end) "
"          from #krm023_dedup s "
"          where #base.idn = s.idn "
"          group by idn) "
"     where exists (select * "
...
" drop table #krm034_dedup;"
" drop table #stm001_dedup;"
" drop table #stm002_dedup;"
" drop table #jas002_t;"
