...
DROP TABLE JAS002_200502_T
GO
DROP TABLE JAS002_200502_T_DEDUP
GO
DROP TABLE TMP
GO
ALTER TABLE KRM023_200502_DEDUP DROP COLUMN BUCKET_DEF_1K
GO
ALTER TABLE KRM023_200502_DEDUP DROP COLUMN BUCKET_EF_1K
GO
ALTER TABLE KRM023_200502_DEDUP DROP COLUMN BUCKET_F_1K
GO
ALTER TABLE KRM023_200502_DEDUP DROP COLUMN PAYMENT_AMT
GO
ALTER TABLE KRM023_200502_DEDUP DROP COLUMN MON_SINCE
GO
ALTER TABLE KRM023_200502_DEDUP ADD BUCKET_DEF_1K INT
GO
ALTER TABLE KRM023_200502_DEDUP ADD BUCKET_EF_1K INT
GO
ALTER TABLE KRM023_200502_DEDUP ADD BUCKET_F_1K INT
GO
ALTER TABLE KRM023_200502_DEDUP ADD PAYMENT_AMT FLOAT
GO
ALTER TABLE KRM023_200502_DEDUP ADD MON_SINCE INT
GO

UPDATE KRM023_200502_DEDUP
SET
...
CREATE TABLE JAS002_200502_T (IDN Char(6), Reason char, Date_Happen char(7), Mon_Since int)
GO
CREATE TABLE JAS002_200502_T_DEDUP (IDN Char(6), Reason char, Date_Happen char(7), Mon_Since int, Records int) 
GO
CREATE TABLE TMP (IDN Char(6), Mon int, V1 int)
GO
CREATE TABLE TMP1 (IDN Char(6), Mon int, V1 int)

INSERT INTO JAS002_200502_T
SELECT IDN, 'D', Delinquent_date, NULL
FROM JAS002_200502_DEDUP
...
SET @i=12 
  ...
  BEGIN
    UPDATE KRM023_200502_DEDUP
    SET  
      bucket_def_1k =(CASE WHEN KRM023_200502_DEDUP.PAY_code IN ('D', 'E', 'F') AND
    	                        KRM023_200502_DEDUP.Payment_Amt > 1 
        	                THEN a.bucket_def_1k + 1 
            		   ELSE 0 END), 
      bucket_ef_1k =(CASE WHEN  KRM023_200502_DEDUP.pay_code in ('E', 'F') AND
                                KRM023_200502_DEDUP.Payment_Amt > 1 
                                THEN a.bucket_ef_1k + 1 
                          ELSE 0 END), 
      bucket_f_1k =(CASE WHEN KRM023_200502_DEDUP.pay_code = 'F' AND
                              KRM023_200502_DEDUP.Payment_Amt > 1 
                         THEN a.bucket_f_1k + 1 
                         ELSE 0 END) 
      FROM KRM023_200502_DEDUP AS A INNER JOIN  KRM023_200502_DEDUP ON 
      A.IDN =  KRM023_200502_DEDUP.IDN AND 
      A.Issue_Name =  KRM023_200502_DEDUP.Issue_Name AND 
      A.Mon_Since = ( KRM023_200502_DEDUP.Mon_Since - 1)
...
ALTER TABLE BAM086_200502_DEDUP ADD BANK_CODE2 CHAR(3)
GO

UPDATE BAM086_200502_DEDUP
SET
BANK_CODE2 = LEFT(LTRIM(BANK_CODE), 3)
GO

alter table BAM086_200502_DEDUP
add
...
go
--68878
drop table y_BAM086_tmp_200502
go
create table y_BAM086_tmp_200502 
(IDN Char(6), 
...
insert into y_BAM086_tmp_200502(IDN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, CO_LOAN, Bank_Code2, Bucket, Cycle)
select
