...
/*-----TF_KHJ-----*/
DROP TABLE Open_Card_200409, Open_Line_200409, Latest_Stmt_Mon_200409, Latest_Line_200409
DROP TABLE MS2xx_200409, FS2xx_200409, TMP2xx, TMP2xx_1, Ind_200409 
GO
...
CREATE TABLE Open_Card_200409
(IDN Char(6), Issue char(3), Mon int)

CREATE TABLE Open_Line_200409
(IDN Char(6), Issue char(3), Mon int, Cards int)
GO

DECLARE @i INT
DECLARE @NOW INT
SET @NOW = 93 * 12 + 9 /*for TF_KHJ*/
SET @i = 0
...
INSERT INTO Open_Line_200409
SELECT IDN, Issue, Mon, Count(*)
FROM Open_Card_200409
...
CREATE INDEX idx ON Open_Line_200409(IDN)
GO

ALTER TABLE Open_Line_200409 ADD Bucket int
GO

UPDATE Open_Line_200409
SET Bucket = 1
Go

UPDATE Open_Line_200409
SET Bucket = 100 /*arbitrary number, just to make sure it will out-of-range*/
WHERE MON = ((93 * 12 + 9) - 13)
GO
...
SET
Bucket = A.Bucket + 1
FROM Open_Line_200409 AS A INNER JOIN Open_Line_200409 ON
      A.IDN = Open_Line_200409.IDN AND
      A.Issue = Open_Line_200409.Issue AND
      A.Mon = (Open_Line_200409.Mon - 1)
WHERE (@NOW - A.Mon) = @i
...
CREATE TABLE Latest_Stmt_Mon_200409 (IDN Char(6), Issue Char(3), Mon Int)
CREATE TABLE Latest_Line_200409 (IDN Char(6), Issue Char(3), Mon Int, MOB Int)
Go

DECLARE @NOW INT
SET @NOW = 93 * 12 + 9 /*for TF_KHJ*/

INSERT INTO Latest_Stmt_Mon_200409
SELECT IDN, Issue, Max(Mon)
FROM Open_Line_200409
WHERE Mon < @NOW
GROUP BY IDN, Issue
GO

CREATE INDEX idx ON Latest_Stmt_Mon_200409(IDN, Issue, Mon)
GO

INSERT INTO Latest_Line_200409
SELECT A.IDN, A.Issue, A.Mon, A.Bucket
FROM Open_Line_200409 AS A INNER JOIN Latest_Stmt_Mon_200409 AS B
ON A.IDN = B.IDN AND
   A.Issue = B.Issue AND
   A.Mon = B.Mon
GO

CREATE INDEX idx ON Latest_Line_200409(IDN, Issue)
GO


/*----Base Table----*/
CREATE TABLE MS2xx_200409
...
INSERT INTO MS2xx_200409(IDN)
SELECT IDN
FROM BASE_200409
GO

INSERT INTO FS2xx_200409(IDN)
SELECT IDN
FROM BASE_200409
GO

CREATE INDEX IDX ON MS2xx_200409(IDN)
CREATE INDEX IDX ON FS2xx_200409(IDN)
GO
...
FROM KRM023_200409 AS A, KRM001_200409 AS B
WHERE A.IDN = B.IDN AND 
      A.IDN = MS2xx_200409.IDN AND
...
BEGIN
  ...
  FROM KRM023_200409 AS A
  WHERE Issue IN
  (SELECT Issue FROM Latest_Line_200409
...
MS201_3M_1k = V1
FROM TMP2xx AS A
WHERE A.IDN = MS2xx_200409.IDN AND
      A.Mon = 3
GO
...
FS201_3M_1k = V1
FROM TMP2xx AS A
WHERE A.IDN = FS2xx_200409.IDN AND
      A.Mon = 3
GO
...
/*---Indicator variables----*/
CREATE TABLE Ind_200409 (IDN Char(6), Ind001 Int, Ind002 Int)
GO

INSERT INTO Ind_200409(IDN)
SELECT IDN
FROM BASE_200409
GO

CREATE INDEX idx ON Ind_200409(IDN)
GO
...
Ind001 = 1
FROM TMP2xx AS A
WHERE A.IDN = Ind_200409.IDN AND
