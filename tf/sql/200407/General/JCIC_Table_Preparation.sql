USE TF_KHJ
GO
DROP TABLE JAS002_200407_T
GO
CREATE TABLE JAS002_200407_T (IDN Char(6), Reason char, Date_Happen char(7), Mon_Since int)
GO
INSERT INTO JAS002_200407_T
...
GO
--8
DROP TABLE JAS002_200407_T_DEDUP
GO
SELECT IDN, Reason, Date_Happen, Mon_Since, COUNT(*) AS Records
INTO JAS002_200407_T_DEDUP
FROM JAS002_200407_T
GROUP BY IDN, Reason, Date_Happen, Mon_Since
GO
--12
SELECT COUNT(*), COUNT(DISTINCT IDN), SUM(RECORDS)
FROM JAS002_200407_T_DEDUP
/*12	10	12*/

ALTER TABLE KRM001_200407 ADD Start_Mon_Since int, End_Mon_Since int
GO
UPDATE KRM001_200407
SET
Start_Date =
(CASE
...
GO
/*945*/
select end_mon_since from krm001_200407 where stop_date is null
select max(stop_date), max(end_mon_since) from krm001_200407 where stop_date is not null
select start_date, start_mon_since from krm001_200407 where start_date like '%*%'

UPDATE JAS002_200407_T_DEDUP
SET
Date_Happen =
(CASE
...
GO
/*12*/
SELECT * FROM JAS002_200407_T_DEDUP
GO

ALTER TABLE KRM023_200407 ADD Mon_Since int
GO
UPDATE KRM023_200407
SET
YRMON =
(CASE
...
GO
/*6053*/
ALTER TABLE STM001_200407 ADD Query_Mon_Since int
GO
UPDATE STM001_200407
SET
Query_Date =
(CASE
...
GO
/*986*/
ALTER TABLE KRM023_200407 ADD Payment_Amt float
GO
UPDATE KRM023_200407
SET
Payment_Amt = 
...
GO
/*6053*/
ALTER TABLE KRM023_200407 ADD Bucket_def_1K INT, Bucket_ef_1K INT, Bucket_f_1K INT;
GO
UPDATE KRM023_200407
SET 
BUCKET_DEF_1k = 0,
...
BEGIN
  UPDATE KRM023_200407
  SET  
  bucket_def_1k =(CASE WHEN KRM023_200407.PAY_code IN ('D', 'E', 'F') AND
                            KRM023_200407.Payment_Amt > 1 
                       THEN a.bucket_def_1k + 1 
                       ELSE 0 END), 
  bucket_ef_1k =(CASE WHEN  KRM023_200407.pay_code in ('E', 'F') AND
                            KRM023_200407.Payment_Amt > 1 
                      THEN a.bucket_ef_1k + 1 
                      ELSE 0 END), 
  bucket_f_1k =(CASE WHEN  KRM023_200407.pay_code = 'F' AND
                           KRM023_200407.Payment_Amt > 1 
                     THEN a.bucket_f_1k + 1 
                     ELSE 0 END) 
  FROM  KRM023_200407 AS A INNER JOIN  KRM023_200407 ON 
        A.IDN =  KRM023_200407.IDN AND 
        A.Issue =  KRM023_200407.Issue AND 
        A.Mon_Since = ( KRM023_200407.Mon_Since - 1) 
  WHERE (@NOW - A.Mon_Since) = @i 
SET @i = @i - 1 
END;
GO

ALTER TABLE BAM085_200407 ADD Bank_Code2 char(3)
GO
UPDATE BAM085_200407
SET
Bank_Code2 = LEFT(BANK_CODE,3)
GO
--653
DROP TABLE BASE_200407
GO
SELECT IDN, MIN_INQUIRY_DATE, ACTUAL_DATA_FLAG AS AVAIL_FLAG, INQUIRY_DATE_DIFF
INTO BASE_200407
FROM data_availability
WHERE JCIC_MONTH = '200407'
GO
--220
