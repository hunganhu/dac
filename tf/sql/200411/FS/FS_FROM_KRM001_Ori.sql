...
select @months = (@now - min(start_mon_since)) from KRM001_200411
...
FROM FS_TMP AS A
WHERE A.IDN = FS_200411.IDN  AND MON = 3

UPDATE FS_200411
SET
FS020_6M = ISNULL(V1,0) + ISNULL(V2,0)
FROM FS_TMP AS A
WHERE A.IDN = FS_200411.IDN  AND MON = 6

UPDATE FS_200411
SET
FS020_9M = ISNULL(V1,0) + ISNULL(V2,0)
FROM FS_TMP AS A
WHERE A.IDN = FS_200411.IDN  AND MON = 9

UPDATE FS_200411
SET
FS020_12M = ISNULL(V1,0) + ISNULL(V2,0)
FROM FS_TMP AS A
WHERE A.IDN = FS_200411.IDN  AND MON = 12
GO
