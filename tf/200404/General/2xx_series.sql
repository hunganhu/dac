...
/*-----TF_KHJ-----*/
DROP TABLE Open_Card_200404, Open_Line_200404, Latest_Stmt_Mon_200404, Latest_Line_200404
DROP TABLE MS2xx_200404, FS2xx_200404, TMP2xx, TMP2xx_1, Ind_200404 
GO
...
CREATE TABLE Open_Card_200404
(IDN Char(6), Issue char(3), Mon int)

CREATE TABLE Open_Line_200404
(IDN Char(6), Issue char(3), Mon int, Cards int)
GO

DECLARE @i INT
DECLARE @NOW INT
SET @NOW = 93 * 12 + 4 /*for TF_KHJ*/
SET @i = 0
...
INSERT INTO Open_Line_200404
SELECT IDN, Issue, Mon, Count(*)
FROM Open_Card_200404
...
CREATE INDEX idx ON Open_Line_200404(IDN)
GO

ALTER TABLE Open_Line_200404 ADD Bucket int
GO

UPDATE Open_Line_200404
SET Bucket = 1
Go

UPDATE Open_Line_200404
SET Bucket = 100 /*arbitrary number, just to make sure it will out-of-range*/
WHERE MON = ((93 * 12 + 4) - 13)
GO
...
SET
Bucket = A.Bucket + 1
FROM Open_Line_200404 AS A INNER JOIN Open_Line_200404 ON
      A.IDN = Open_Line_200404.IDN AND
      A.Issue = Open_Line_200404.Issue AND
      A.Mon = (Open_Line_200404.Mon - 1)
WHERE (@NOW - A.Mon) = @i
...
CREATE TABLE Latest_Stmt_Mon_200404 (IDN Char(6), Issue Char(3), Mon Int)
CREATE TABLE Latest_Line_200404 (IDN Char(6), Issue Char(3), Mon Int, MOB Int)
Go

DECLARE @NOW INT
SET @NOW = 93 * 12 + 4 /*for TF_KHJ*/

INSERT INTO Latest_Stmt_Mon_200404
SELECT IDN, Issue, Max(Mon)
FROM Open_Line_200404
WHERE Mon < @NOW
GROUP BY IDN, Issue
GO

CREATE INDEX idx ON Latest_Stmt_Mon_200404(IDN, Issue, Mon)
GO

INSERT INTO Latest_Line_200404
SELECT A.IDN, A.Issue, A.Mon, A.Bucket
FROM Open_Line_200404 AS A INNER JOIN Latest_Stmt_Mon_200404 AS B
ON A.IDN = B.IDN AND
   A.Issue = B.Issue AND
   A.Mon = B.Mon
GO

CREATE INDEX idx ON Latest_Line_200404(IDN, Issue)
GO


/*----Base Table----*/
CREATE TABLE MS2xx_200404
...
INSERT INTO MS2xx_200404(IDN)
SELECT IDN
FROM BASE_200404
GO

INSERT INTO FS2xx_200404(IDN)
SELECT IDN
FROM BASE_200404
GO

CREATE INDEX IDX ON MS2xx_200404(IDN)
CREATE INDEX IDX ON FS2xx_200404(IDN)
GO
...
FROM KRM023_200404 AS A, KRM001_200404 AS B
WHERE A.IDN = B.IDN AND 
      A.IDN = MS2xx_200404.IDN AND
...
BEGIN
  ...
  WHERE Issue IN
  (SELECT Issue FROM Latest_Line_200404
...
MS201_3M_1k = V1
FROM TMP2xx AS A
WHERE A.IDN = MS2xx_200404.IDN AND
      A.Mon = 3
GO
...
FS201_3M_1k = V1
FROM TMP2xx AS A
WHERE A.IDN = FS2xx_200404.IDN AND
      A.Mon = 3
GO
...
/*---Indicator variables----*/
CREATE TABLE Ind_200404 (IDN Char(6), Ind001 Int, Ind002 Int)
GO

INSERT INTO Ind_200404(IDN)
SELECT IDN
FROM BASE_200404
GO

CREATE INDEX idx ON Ind_200404(IDN)
GO
...
Ind001 = 1
FROM TMP2xx AS A
WHERE A.IDN = Ind_200404.IDN AND
