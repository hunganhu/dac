...
*/
 ...
 update tf_ploan_cal
    set demo_score =  0.07161	+
		      ...
		      fs029_tran0	*  0.0257;		              
 update tf_ploan_cal
    set demo_twentile = (case when demo_score <= 0.070500713 then 0
                              when demo_score <= 0.07567789  then 1
                              when demo_score <= 0.080793804 then 2
                              when demo_score <= 0.08369692  then 3
                              when demo_score <= 0.086490683 then 4
                              when demo_score <= 0.092223057 then 5
                              when demo_score <= 0.096807994 then 6
                              when demo_score <= 0.108087593 then 7
                              when demo_score <= 0.121242202 then 8
                              when demo_score <= 0.127603141 then 9
                              when demo_score <= 0.140645667 then 10
                              when demo_score <= 0.199175101 then 11
                              when demo_score <= 0.297444698 then 12
                              when demo_score <= 0.363314142 then 13
                              else 14 end),
        score_card = 4;
go
...
*/
 ...
 update tf_ploan_cal
    set mp = isnull(monthly_payment, 0) + isnull(ms082, 0) * 1000.0;
 ...
 update tf_ploan_cal
    set b2_twentile = (case when b2_score <= 0.134945665 then 0
...
*/
 ...
 update tf_ploan_cal
    set b1_score = 0.12288	+
		...
		fs313_tran2	* 0.08463;
 /*twentile 3 is missing */
 update tf_ploan_cal
    set b1_twentile = (case when b1_score <= 0.031130021 then 0
                            when b1_score <= 0.058291646 then 1
                            when b1_score <= 0.085505617 then 2
                            when b1_score <= 0.119389694 then 4
                            when b1_score <= 0.137359645 then 5
                            when b1_score <= 0.156121074 then 6
                            when b1_score <= 0.181768755 then 7
                            when b1_score <= 0.201005969 then 8
                            when b1_score <= 0.213969007 then 9
                            when b1_score <= 0.230702779 then 10
                            when b1_score <= 0.248090634 then 11
                            when b1_score <= 0.260996253 then 12
                            when b1_score <= 0.274342634 then 13
                            when b1_score <= 0.289082445 then 14
                            when b1_score <= 0.303299903 then 15
                            when b1_score <= 0.317342423 then 16
                            when b1_score <= 0.330608749 then 17
                            when b1_score <= 0.343733214 then 18
                            else 19 end),
        score_card = 2;
 go
...
*/
 ...
 update tf_ploan_cal
    set monthly_payment = principal * amortization_rate;
 update tf_ploan_cal
    set LN001_12M = (isnull(monthly_payment, 0) + isnull(ms082, 0) * 1000.0) /
 ...
 update tf_ploan_cal
    set full_twentile = (case when full_score <= 0.02011206 then 0
...
where jcic_date is null
  ...
  update loan_condition
     set apr = (case when apr = 0 then null else apr / 100.0 end),
         terms = (case when terms =0 then null else terms end)
...
/* select approciated model: demo, bam085_1,bam085_2, full jcic */
 ...
 update tf_ploan_cal_batch set score_card = null

 update tf_ploan_cal_batch
 ...
 update tf_ploan_cal
    set return_msg = NULL;
 update tf_ploan_cal
    set return_msg = (case when jas002_defect > 0 then '104 - 有退票強停拒往授信異常等記錄'
                           when app_max_bucket > 3 then '105 - 信用卡有90 天以上遲繳記錄'
                           when fs044 > 0 then '106 - 貸款有遲繳記錄'
                           when fs334 > 3 then '108 - 貸款有90 天以上遲繳記錄'
