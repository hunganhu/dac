...
/* 
 create index i_krm001 on krm001_dedup(app_sn);
 create index i_krm023 on krm023_dedup(app_sn);
 create index i_bam085 on bam085_dedup(app_sn);
 create index i_jas002t on jas002_t_dedup(app_sn);
 create index i_stm001 on stm001_dedup(app_sn);
*/ 
 update krm001_dedup
    set start_date = (case
 ...
 while @i > 0
    begin
       update krm023_dedup
          ...
          from krm023_dedup, krm023_dedup as a, daco_v41_cal as b
...
alter PROCEDURE TF_loan_prescreen
 ...
 update daco_v41_cal
    set avail_flag = a.v1
    from #tmp as a
    where a.app_sn = daco_v41_cal.app_sn;
...
/* jas002_defect  = 1 */
 ...
 insert into #tmp (app_sn, v1)
    ...
    from jas002_t_dedup a, daco_v41_cal b
...
/* FS334 > 3 曾經最高逾期月數*/
 ...
 update daco_v41_cal
    ...
    from #tmp as a
    where a.app_sn = daco_v41_cal.app_sn
      and (daco_v41_cal.now - a.mon) between 1 and 7;
...
--       where a.app_sn = daco_v41_cal.app_sn
 ...
 insert into #tmp (app_sn, v1)
    select a.app_sn, count(*)
    from stm001_dedup a, daco_v41_cal b
 ...
 /* Transformation */
 update daco_v41_cal
    set education = (case when edu = '研究所(含)以上' then 1
			  when edu = '大學'           then 2
			  when edu = '專科'           then 3
 ...
 update daco_v41_cal
    set fs029_tran0 =(case when fs029 < 1 then 1
        ...
        gender_tran0 = (case when gender = '男' then 1
			    else 0 end),
        secret_tran0 = (case when is_secretive = '是' then 1
			     else 0 end),
        single_0 = (case when martial_status = '未婚' then 1
			    else 0 end),
	education_tran0 = (case when education < 4 then 1 /* 專科以上 */
				else 0 end);
    
 update daco_v41_cal
    set demo_score =  0.05401	      +       
		      secret_tran0    * -0.08681 +
		      gender_tran0    *  0.07774 +       
...
--    where a.app_sn = daco_v41_cal.app_sn
 update  daco_v41_cal
    ...
    where a.app_sn = daco_v41_cal.app_sn
 /*****************************************************************************************/
 /* start making fs031 JCIC查詢次數 (All) (stm001)within 12 month                        */
 /*****************************************************************************************/
 delete from #tmp
 insert into #tmp (app_sn, v1)
    select a.app_sn, count(*)
...
--    where a.app_sn = daco_v41_cal.app_sn
 ...
 update daco_v41_cal
    ...
    where a.app_sn = b.app_sn
      and a.app_sn = daco_v41_cal.app_sn
      and ((a.issue = b.issue) 
 ...
 update daco_v41_cal
    ...
    where a.app_sn = daco_v41_cal.app_sn
 /*****************************************************************************************/
 /* fs059_1k 有延遲繳款額度數 (M1+) - Pay code in ('E','F')                            */
 /*****************************************************************************************/
 delete from #tmp
 set @i = 3
 insert into #tmp (app_sn, mon, v1)
    select a.app_sn, 3, count(distinct issue)
    from krm023_dedup a, daco_v41_cal b
 ...
 update daco_v41_cal
    set fs059_3m_1k = v1
    from #tmp as a
    where a.app_sn = daco_v41_cal.app_sn and mon = 3;
 ...
 /*****************************************************************************************/
 /* MS024  持有之信用卡平均額度  For line > 0 only                                        */
 /*****************************************************************************************/
 delete from #tmp;
 insert into #tmp (app_sn, mon, v1)
    ...
    begin
       insert into #open_card (app_sn, issue, mon)
          select a.app_sn,
                 ...
                 (b.now - @i)
          from krm001_dedup a, daco_v41_cal b
 ...
 while @i > 0
    begin
       update #open_line
          ...
          from #open_line, #open_line as a, daco_v41_cal as b
 ...
 insert into #latest_stmt_mon (app_sn, issue, mon)
    ...
    from #open_line a, daco_v41_cal b
 ...
 insert into #tmp (app_sn, mon, v1)
    ...
    from krm023_dedup as a, daco_v41_cal b
 ...
 update daco_v41_cal
    set fs203_12m_1k = v1
    from #tmp as a
    where a.app_sn = daco_v41_cal.app_sn and
          a.mon = 12;
 ...
 update daco_v41_cal
    set ms056_6m_1k = v1
    from #tmp as a
    where a.app_sn = daco_v41_cal.app_sn and mon=6;
 ...
 update daco_v41_cal
    set fs014_12m = v1
    from #tmp as a
    where a.app_sn = daco_v41_cal.app_sn and mon=12;
 ...
 insert into #tmp(app_sn, v1)
    select idn,
       loan1_interest_payment6 + loan2_interest_payment6 + loan3_interest_payment6
     + loan1_interest_payment5 + loan2_interest_payment5 + loan3_interest_payment5
     + loan1_interest_payment4 + loan2_interest_payment4 + loan3_interest_payment4
     + loan1_interest_payment3 + loan2_interest_payment3 + loan3_interest_payment3
     + loan1_interest_payment2 + loan2_interest_payment2 + loan3_interest_payment2
     + loan1_interest_payment1 + loan2_interest_payment1 + loan3_interest_payment1
    from application
 update daco_v41_cal
 ...
 update daco_v41_cal
    set fs031_tran3 =(case when fs031_r is null then -1
			   when fs031_r > 12 then 12
      			   else fs031 end),
...
/*
if exists (select * from dbo.sysobjects where id = object_id(N'bam101_dedup') and objectproperty(id, N'isusertable') = 1)
   drop table bam101_dedup;
create table bam101_dedup (
   app_sn char(11),
   data_time char(8),
   data_yyy char(3),
   data_mm char(2),
...
create table stm001_dedup (
   ...
   query_date char(7),
   bank_code char(7),
   bank_name char(40),/* Get date from input tables*/
...
if exists (select * from dbo.sysobjects where id = object_id(N'daco_v41_cal') and objectproperty(id, N'isusertable') = 1)
...
/* load application data */
  insert into daco_v41_cal (app_sn, edu, gender, is_secretive, martial_status, apr, periods, principal, inquiry_date)
     select idn, edu, gender, is_secretive, martial_status, interest_rate, periods, actual_amount, inquiry_date
     from application
  update daco_v41_cal
     set now = (convert (int, substring(inquiry_date, 1, 4)) - 1911) * 12
                     ...
                     WHERE CURRENT OF record_cur
                        ...
                        update daco_v41_cal
                          ...
                          update daco_v41_cal
                            set return_msg = '107 - 現金卡使用超出額度'
                            WHERE CURRENT OF record_cur
      if @krm001_hit = 1 and @krm023_hit = 1 and @ind001 = 1 
         exec TF_ploan_model 1118     /* @now 93/02 */
      else if @bam085_hit = 1 and @ms080 <= 0
              exec TF_BAM_no_payment 1118     /* @now 93/02 */
           else if @bam085_hit = 1 and @ms080 > 0
