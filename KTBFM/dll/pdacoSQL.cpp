...
" MONTHLY_PAYMENT, BRANCH, EMP_ID, AUDITOR, APP_INQ_DATE, COS_INQ_DATE, GUA_INQ_DATE "
" from APP_INFO"
" where CASE_NO = :v0;",

/*Get_Prescreen_Result*/
" select JAS002_DEFECT, KRM021_HIT, KRM023_HIT, BAM086_HIT, MAX_BUCKET, FS044, FS059_1K_12M,"
" FS302, FS334, CARD_FORCE_STOP, CASH_MAX_BUCKET, CASH_UTILIZATION, IND001, REVOLVING_AMT, MS101"
...
"	MS093		DECIMAL(16,8) DEFAULT 0.0,"
"	MS094		DECIMAL(16,8) DEFAULT 0.0,"
"	MS105		DECIMAL(16,8) DEFAULT 0.0,"
"	MS101		DECIMAL(16,8) DEFAULT 0.0);"
,
/* DROP_WORKING_TABLES */
" DROP TABLE #BAM086_DEDUP;"
" DROP TABLE #BAM086_BUCKET;"
...
"INSERT INTO #BAM087_DEDUP(CASE_NO, INQUIRY_DATE, IDN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT1, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, PAY_CODE_12, IS_KIND, PROJECT_CODE, CO_LOAN, UN_MARK, U_YYYMMDD, U_RATE, IB_MARK, IAB_BAN, IAB_NAME, CONTRACT_MARK, CONTRACT_CODE, CONTRACT_CODE1, CON_BAN, CON_NAME, ACT_Y_MARK, CONTRACT_AMT_Y, CNT)"
"   SELECT CASE_NO, INQUIRY_DATE, IDN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CAST(CONTRACT_AMT1 AS INT), CAST(CONTRACT_AMT AS INT), CAST(LOAN_AMT AS INT), CAST(PASS_DUE_AMT AS INT), PAY_CODE_12, IS_KIND, PROJECT_CODE, CO_LOAN, UN_MARK, U_YYYMMDD, U_RATE, IB_MARK, IAB_BAN, IAB_NAME, CONTRACT_MARK, CONTRACT_CODE, CONTRACT_CODE1, CON_BAN, CON_NAME, ACT_Y_MARK, CAST(CONTRACT_AMT_Y AS INT), COUNT(*)"
"   FROM BAM087"
"   WHERE CASE_NO = @CASE_NO AND IDN = @IDN"
"   GROUP BY CASE_NO, INQUIRY_DATE, IDN, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT1, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, PAY_CODE_12, IS_KIND, PROJECT_CODE, CO_LOAN, UN_MARK, U_YYYMMDD, U_RATE, IB_MARK, IAB_BAN, IAB_NAME, CONTRACT_MARK, CONTRACT_CODE, CONTRACT_CODE1, CON_BAN, CON_NAME, ACT_Y_MARK, CONTRACT_AMT_Y; "
"IF @@ROWCOUNT > 0 "
"   BEGIN "
"      UPDATE #BAM087_DEDUP"
...
"      INSERT INTO #BAM086_DEDUP (CASE_NO, IDN, INQUIRY_DATE, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, PAY_CODE_12, CO_LOAN)"
"        SELECT CASE_NO, IDN, INQUIRY_DATE, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, PAY_CODE_12, CO_LOAN"
"        FROM #BAM087_DEDUP "
"        GROUP BY CASE_NO, IDN, INQUIRY_DATE, DATA_YYY, DATA_MM, BANK_CODE, BANK_NAME, ACCOUNT_CODE, ACCOUNT_CODE2, PURPOSE_CODE, CONTRACT_AMT, LOAN_AMT, PASS_DUE_AMT, PAY_CODE_12, CO_LOAN; "
"   END "
"ELSE "
"   BEGIN "
...
"   END "
" INSERT INTO #KRM037_DEDUP(CASE_NO, INQUIRY_DATE, IDN, IDN_BAN, BILL_DATE, ISSUE, ISSUE_NAME, CARD_TYPE, PERM_LIMIT, TEMP_LIMIT, CASH_LIMIT, PAYABLE, CASH_LENT, LAST_PAYA, REVOL_BAL, PAY_STAT, PAY_CODE, REVOL_RATE, PRE_OWED, DEBT_CODE, CLOSE_CODE, CLEAR_DATE)"
...
"          ISSUE_NAME, PERM_LIMIT, LAST_PAYA, CONVERT(INT, REVOL_BAL) / 1000.0,"
"          (CASE WHEN CONVERT(INT, CASH_LENT) > 0 THEN 'Y' ELSE 'N' END),"
"          (CASE WHEN PAY_STAT = 'X' AND PAY_CODE = 'X' THEN 'X'"
"                WHEN DEBT_CODE IN ('A', 'B') THEN 'F'"
...
"   WHERE A.IDN = #PDACO_V1_00_CAL.IDN;"
" INSERT INTO #TMP1 (IDN, V1)"
"   SELECT IDN, (CASE WHEN (@NOW - MIN(START_MON_SINCE)) < 7 THEN 1 ELSE 0 END)"
"   FROM #KRM021_DEDUP"
