...
/* CREATE_PROCEDURE_PREPARE_JCIC_DATA */
 ...
 INSERT INTO #KRM023_TMP (CASE_NO, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT)
   SELECT CASE_NO, IDN,
          (CASE WHEN LEFT(BILL_DATE,1) = '*' THEN NULL
                ...
                WHEN LEN(BILL_DATE) = 7 AND CONVERT(INT, RIGHT(BILL_DATE,2)) > 15
                     THEN CONVERT(INT, LEFT(BILL_DATE, 3)) * 12 + CONVERT(INT, SUBSTRING(BILL_DATE, 4, 2))
                WHEN LEN(BILL_DATE) = 7
                     THEN CONVERT(INT,LEFT(BILL_DATE, 3)) * 12 + CONVERT(INT, SUBSTRING(BILL_DATE, 4, 2)) - 1
                ELSE NULL END),
 ...
 INSERT INTO #KRM023_TMP1 (CASE_SN, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, CASH, PAY_CODE, SPREAD_PAYMENT)
   SELECT CASE_SN, IDN, MON_SINCE, ISSUE, ISSUE_NAME, SUM(CONVERT(INT, LIMIT)), SUM(PAYMENT_AMT),
...
UPDATE #KRM021_DEDUP
 ...
 UPDATE #KRM023_DEDUP
  SET PAYMENT_AMT =
   ...
   ELSE 0 END) * POWER(10, ISNULL(LEFT(PAYMENT,2),0)-1) / 1000.0;
 INSERT INTO #KRM023_DEDUP (CASE_NO, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, CASH, PAY_CODE)
   SELECT CASE_NO, IDN, MON_SINCE, ISSUE, ISSUE_NAME, LIMIT, PAYMENT_AMT, CASH, PAY_CODE
   FROM #KRM023_TMP1
 ...
 UPDATE #PDACO_V1_00_CAL
   SET KRM023_HIT = 1
   WHERE AVAIL_FLAG & 0X02 = 0X01;
 UPDATE #PDACO_V1_00_CAL
   SET KRM021_HIT = 1
   WHERE AVAIL_FLAG & 0X01 = 0X02;
 UPDATE #PDACO_V1_00_CAL
   SET BAM086_HIT = 1
...
CREATE PROCEDURE FWPL_PDACO_RSCORE
 AS

DECLARE @CASE_NO VARCHAR(20)
DECLARE @IDN     CHAR(11)
...
SELECT CASE_NO, IDN, INQUIRY_DATE
FROM APP_INFO

OPEN CURSOR1

FETCH NEXT FROM CURSOR1
INTO @CASE_NO, @IDN, @INQUIRY_DATE
...
BEGIN
   ...
   EXEC PREPARE_PDACO_JCIC_DATA @CASE_NO, @IDN,@NOW
   EXEC GENERATE_PDACO_SCORE @NOW, @CURR_INQMON

   INSERT INTO PDACO_SCORE_NEW_DEFINITION
     SELECT * FROM #PDACO_V1_00_CAL
...
/*
 ...
 DROP TABLE #BASE;
 DROP TABLE #PDACO_V1_00_CAL;
 DELETE FROM OLIVER.DAC_AUDIT

SELECT COUNT(*) FROM OLIVER.DAC_AUDIT

DELETE FROM DBO.PDACO_SCORE_NEW_DEFINITION
*/

-- EXEC FWPL_PDACO_RSCORE
